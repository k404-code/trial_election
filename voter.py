...
def cast_vote():
    ...
    try:
        candidate_number = int(input("Enter the Sl. No of the candidate you want to vote for: "))
        if 1 <= candidate_number <= len(candidates):
            selected_candidate = candidates[candidate_number - 1]
            print(f"You have voted for: {selected_candidate['name']} from {selected_candidate['forum']}")
        else:
